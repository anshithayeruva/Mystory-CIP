...
export default function HodFacultyPage() {
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const parsedHours = Math.max(1, parseInt(formData.teachingHours, 10) || 12);
    ...
    if (editingId) {
        ...
        prev.map((f) =>
          f.id === editingId
            ? {
                ...
                sections: formData.sections || "02",
                students: formData.students || "100",
                teachingHours: parsedHours,
              }
            : f
        )
      );
      ...
      // Add new member
      const newFacultyMember: FacultyItem = {
      ...
      setFacultyList((prev) => [newFacultyMember, ...prev]);
      setCurrentPage(1);
    }
  ...
  const filteredFaculty = facultyList.filter((item) => {
    ...
    const matchesSubject =
      subjectFilter === "ALL" ||
      item.subjects.some((s) => s.toLowerCase() === subjectFilter.toLowerCase());

    let matchesHours = true;
    if (hoursFilter === "0-8") matchesHours = item.teachingHours >= 0 && item.teachingHours <= 8;
            ...
            {isFilterOpen && (
              <div className={styles.filterDropdown}>
                ...
                <div className={styles.filterGroup}>
                  <label className={styles.filterLabel}>Academic Rank / Role</label>
                  <select
